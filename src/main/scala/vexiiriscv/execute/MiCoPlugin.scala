...
object MiCoPlugin {
    ...
    val AQ = Payload(UInt(4 bits))  // 1 2 4 8
    val WQ = Payload(UInt(4 bits))  // 1 2 4 8
...
class MiCoPlugin(val layer : LaneLayer) 
    ...
    val logic = during setup new Logic {
        ...
        add(DOTP8x8).srcs(SRC1.RF, SRC2.RF).decode(AQ -> U(8), WQ -> U(8), INC -> U(0))
        add(DOTP8x4).srcs(SRC1.RF, SRC2.RF).decode(AQ -> U(8), WQ -> U(4), INC -> U(4))
        add(DOTP8x2).srcs(SRC1.RF, SRC2.RF).decode(AQ -> U(8), WQ -> U(2), INC -> U(8))
        add(DOTP8x1).srcs(SRC1.RF, SRC2.RF).decode(AQ -> U(8), WQ -> U(1), INC -> U(16))

        // 4-bit Engine Data Path
        add(DOTP4x4).srcs(SRC1.RF, SRC2.RF).decode(AQ -> U(4), WQ -> U(4), INC -> U(0))
        add(DOTP4x2).srcs(SRC1.RF, SRC2.RF).decode(AQ -> U(4), WQ -> U(2), INC -> U(8))
        add(DOTP4x1).srcs(SRC1.RF, SRC2.RF).decode(AQ -> U(4), WQ -> U(1), INC -> U(16))

        // 2-bit Engine Data Path
        add(DOTP2x2).srcs(SRC1.RF, SRC2.RF).decode(AQ -> U(2), WQ -> U(2), INC -> U(0))
        add(DOTP2x1).srcs(SRC1.RF, SRC2.RF).decode(AQ -> U(2), WQ -> U(1), INC -> U(16))

        // 1-bit Engine Data Path
        add(DOTP1x1).srcs(SRC1.RF, SRC2.RF).decode(AQ -> U(1), WQ -> U(1), INC -> U(0))
        ...
        val process = new el.Execute(id = 0) {
            ...
            val ToDOTP8 = WQ.mux(
                ...
                U(4) -> ExtendTo8b(W16b),
                U(2) -> ExtendTo8b(W8b),
                U(1) -> ExtendTo8b(Extend1bTo2b(W4b)),
                default -> B(0) // Invalid
            )
            val ToDOTP4 = WQ.mux(
                U(4) -> W32b,
                U(2) -> Extend2bTo4b(W16b),
                U(1) -> Extend2bTo4b(Extend1bTo2b(W8b)),
                default -> B(0) // Invalid
            )
            val ToDOTP2 = WQ.mux(
                U(2) -> W32b,
                U(1) -> Extend1bTo2b(W16b),
                default -> B(0) // Invalid
            )
            val ToDOTP1 = WQ.mux(
                U(1) -> W32b,
            ...
            val result = AQ.mux(
                U(8) -> DotProduct(rs1, ToDOTP8, 8),
                U(4) -> DotProduct(rs1, ToDOTP4, 4),
                U(2) -> DotProductSym2Bit(rs1, ToDOTP2),
                U(1) -> DotProductSym1Bit(rs1, ToDOTP1),
                default -> S(0) // Invalid
            )
